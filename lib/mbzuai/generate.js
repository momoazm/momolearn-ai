...
export async function generateQuestions(callModel, opts, helpers = {}) {
  ...
  const usable = hasKey ? chain.filter((e) => hasKey(e.provider)) : chain;
  for (const entry of usable.slice(0, maxModels)) {
...
export async function askStructured(callModel, messages, helpers = {}, opts = {}) {
  const { hasKey, isCoolingDown, markCooldown } = helpers;
  const { preferredProvider, deadline } = opts;
  let { chain } = fullRoute('reasoning');
  if (preferredProvider) {
    chain = [...chain.filter((e) => e.provider === preferredProvider), ...chain.filter((e) => e.provider !== preferredProvider)];
  }
  let lastErr = 'no model available';
  const usable = hasKey ? chain.filter((e) => hasKey(e.provider)) : chain;
  for (const entry of usable.slice(0, opts.maxModels ?? 4)) {
    if (deadline && Date.now() > deadline) break;
    if (isCoolingDown && isCoolingDown(entry.label)) continue;
    try {
      const text = await callModel(entry, messages, { temperature: 0.4 });
  ...
  throw Object.assign(new Error('AI request failed. ' + lastErr), { attempts: [lastErr] });
}
